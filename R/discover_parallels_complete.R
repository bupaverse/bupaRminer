...
discover_parallels_complete <- function(eventlog, all, case_count_list, potential = NULL) {
  ...
  for(i in 1:(length(all)-1)) {
    if(all[i] %in% act_a) {
      ...
      output[[i]] <- discover_parallels_one_to_many(eventlog, all[i], act_b[-1:-which(act_b == all[i])],case_count_list = case_count_list)
